...
const workflows = [
  ...
  { to: '', icon: '🏦', title: 'FIS Horizon Core Integration', subtitle: 'Coming soon!', desc: 'FIS Horizon integration mapping configuration' },
        ...
        <div className="button-grid">
          {workflows.map((w, i) =>
            w.to ? (
              ...
              <div key={i} className="landing-btn" style={{ opacity: 0.5, cursor: 'not-allowed' }}>
                <div className="landing-btn-icon">{w.icon}</div>
                <div className="landing-btn-title">{w.title}</div>
                {w.subtitle && <div className="landing-btn-title">{w.subtitle}</div>}
                <div className="landing-btn-desc">{w.desc}</div>
