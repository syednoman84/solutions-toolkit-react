...
def automate(params):
    ...
    # --- Discover products dynamically from params ---
    discovered = []
    ...
    try:
        # --- Step 1 - Clone destination repo (existing tenant) ---
        ...
        # --- Step 3 - Ensure product directory exists ---
        product_dir = dest_repo / "product"
        if not product_dir.exists():
            product_dir.mkdir()
            logging.info(f"Created product directory at {product_dir}")
        else:
            logging.info(f"Product directory already exists at {product_dir} with existing products")
            existing_count = sum(1 for p in product_dir.iterdir() if p.is_dir() and (p / "definition.json").exists())
            logging.info(f"Found {existing_count} existing product(s)")

        # --- Step 4 - Clone template vars repo ---
        ...
        for ptype, idx, key, product_name in discovered:
            ...
            # Create Product Directory with random UUID
            ...
            # Copy template vars into product directory
        ...
        run_cmd(["git", "commit", "-m", params.get("commit_message", "Add new PCM products to existing tenant by automation script")], cwd=dest_repo)
        run_cmd(["git", "push", "--set-upstream", "origin", branch_name], cwd=dest_repo)

        # --- Step 7 - Create Pull Request ---
        pr_title = params.get("pr_title", "Add new products to existing tenant")
        logging.info(f"Creating Pull Request: {branch_name} -> master")
        run_cmd(
            ["gh", "pr", "create", "--base", "master", "--head", branch_name,
             "--title", pr_title, "--body",
             "PR created by automation script to add new PCM products to existing tenant"],
            cwd=dest_repo,
        )

        logging.info("✅ Automation completed successfully.")
        if summary:
            logging.info("Summary of newly added products:")
        ...
        # --- Conditionally delete everything from working directory ---
