...
- **New Tenant Setup** — Create a full tenant config from scratch: clone repos, create product directories, generate `definition.json`, commit, push, and open a PR
...
- **View Products by Tenant** — Fetch and display all product definitions for a tenant in a table
...
```
Browser → React (Vite :3000) → Flask API (:5000) → Python Scripts → Git/GitHub
```
...
| `tenant-domain` | 3-letter tenant code |
| `tenant-id` | Tenant identifier |
| `template_vars_tag` | Version tag for template vars |
